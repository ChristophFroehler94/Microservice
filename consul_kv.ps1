...
<#
  Zweck:
    - Liest advertise_addr aus Consul server.hcl
    - Stellt sicher, dass in .\tlscerts ein gültiges Dev-PFX existiert (dotnet dev-certs)
    - Setzt gemeinsame ENV für beide Services (CONSUL_*, APP_TLS_*)
    - Schreibt KV-Einträge für camera/<Node>/config.json und polflash/<Node>/config.json:
        { "Service": { "AdvertisedHost": "<advertise_addr>" } }

  Hinweise:
    - Keine globalen SERVICE_PREFIX / CONSUL_KVPATH gesetzt → beide Program.cs verwenden Defaults.
    - Beide Microservices + Consul laufen auf demselben Rechner (HTTPS-Port aus server.hcl).
#>
...
function Test-PfxValid($fullpath, [string] $pwd) {
    try {
        ...
        $now = [DateTime]::UtcNow
        return ($cert.NotBefore -le $now -and $cert.NotAfter -ge $now)
    }
    catch { return $false }
}

# Finde existierende PFX
$pfx = Get-ChildItem -LiteralPath $tlsDir -Filter *.pfx -File | Select-Object -First 1
$TlsPfxPath = $null
$TlsPfxPassword = "default"

if ($pfx -and (Test-PfxValid $pfx.FullName $TlsPfxPassword)) {
    Ok "PFX ist gültig: $($pfx.FullName)"
    $TlsPfxPath = $pfx.FullName
} else {
    if ($pfx) { Warn "Gefundene PFX ist ungültig oder abgelaufen. Erzeuge neues Dev-Zertifikat." }
    else      { Info "Keine PFX vorhanden. Erzeuge Dev-Zertifikat." }
    ...
    & dotnet dev-certs https -ep $exportPath -p $TlsPfxPassword | Out-Null
...
$envMap = @{
    ...
    # Gemeinsame TLS-ENV für beide Services
...
# --- KV schreiben: camera/<Node>/config.json und polflash/<Node>/config.json ---
...
foreach ($prefix in $servicePrefixes) {
    $key = "$prefix/$node/config.json"
    $tmp = New-TemporaryFile
    try {
        [System.IO.File]::WriteAllText($tmp, $kvJson, [System.Text.UTF8Encoding]::new($false))
        $fileArg = '@' + $tmp
        & $ConsulExePath kv put $key $fileArg | Out-Null
        if ($LASTEXITCODE -ne 0) { Fail "consul kv put fehlgeschlagen für $key (ExitCode=$LASTEXITCODE)" }
...
try {
    ...
    foreach ($prefix in $servicePrefixes) {
        & $ConsulExePath kv get "$prefix/$node/config.json" | Select-Object -First 10 | Write-Host
    }
